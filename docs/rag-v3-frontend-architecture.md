...
后续版本已新增 `POST /api/v2/sessions/{id}/messages/stream`，使用 fetch 读取 SSE，因为请求需要 POST body 和 X-User-ID；V1 的 stream 接口仍返回 501，不能复用。V3.0 原始范围中的“同步更新”仍指当时的非流式行为。

当前 SSE 包括 `run_started`、`answer_start`、`answer_delta`、`context_usage`、`usage`、`done`、`error`。每个事件带 run_id、递增 sequence；done 在最终引用校验并持久化成功后发送，包含权威完整回答。缓存数据仅在上游报告时更新，不承诺逐 token 获得命中率。

Agent 以 JSON 输出并可能补检索、修复或降级。服务端增量解析 `answer` 字段，只发送临时回答正文；新一版回答用 `answer_start` 通知前端清空旧内容，done 再以校验和持久化后的权威答案覆盖。工具调用、JSON 包装和内部推理不会进入回答流。断线仍不自动重放 POST，需后续 run 状态/幂等支持才可保证恢复。
...
V3.0 原始交付范围为阶段 A–D：两个页面、文件夹上传、搜索删除、资料可用性、非流式问答、随回答同步返回真实指标、下轮估算和手动压缩。后续版本已经实现 SSE 增量答案；断线续跑、正式认证及持久化任务队列仍未实现。
...
长问答的浏览器/代理超时应大于后端 agent_timeout 并留响应余量。SSE 响应已发送禁止缓冲头，部署的反向代理仍须关闭响应缓冲。任何 `VITE_*` 都应视为公开信息，不能放服务端密钥。
...
| 后续增强 | V2 SSE 已实现；之后再设计 run 状态和幂等恢复 | 流式路由、工作流 | 已验收生成中实时反馈，断线恢复仍未实现 |
...
完成缓存统计、支持空会话的估算、逐模型调用预算执行和手动压缩；所有指标从后端读取且注明完整性。通过非流式响应同步更新本轮统计后，完成 V3.0。SSE 已在后续版本实现，断线恢复仍是独立增强。
...
V3.0 已创建 12 个前端源文件并接通 FastAPI，完成对话、资料管理、文件夹队列、引用、运行指标、上下文估算与手动压缩。后续版本已落地 SSE 增量答案；正式认证、多进程任务租约和断线恢复仍按本文边界留作后续增强。
