...
export const useAppStore=defineStore('app',()=>{
  ...
  async function send(id:string,body:any){busy.value=true;streamStatus.value='正在连接';const g=generation.value,now=new Date().toISOString(),key=Date.now(),assistantId=`pending-assistant-${key}`;messages.value.push({id:`pending-user-${key}`,role:'user',content:body.question,model_provider:null,model_name:null,citations:[],token_usage:null,run_metadata:null,created_at:now},{id:assistantId,role:'assistant',content:'',model_provider:null,model_name:null,citations:[],token_usage:null,run_metadata:null,created_at:now});try{const value=await guard(()=>api.chatStream(userId.value,id,body,(event,data)=>{if(generation.value!==g)return;const assistant=messages.value.find(message=>message.id===assistantId);if(!assistant)return;if(event==='answer_start')assistant.content='';else if(event==='answer_delta')assistant.content+=data.delta||'';else if(event==='done'){assistant.content=data.answer;assistant.model_provider=data.model_provider;assistant.model_name=data.model_name;assistant.citations=data.citations||[];assistant.token_usage=data.token_usage;assistant.run_metadata=data.run_metadata}streamStatus.value=event==='run_started'?'正在检索并生成回答':event==='answer_delta'?'正在生成回答':event==='context_usage'?'正在汇总上下文':event==='usage'?'正在保存回答':streamStatus.value}));if(value)await loadSession(id);await loadSessions();return value}catch(caught:any){const message=caught?.message||'请求失败';if(generation.value===g){await loadSession(id).catch(()=>{});error.value=message}throw caught}finally{busy.value=false;streamStatus.value=''}}
